...
async function notifyBookingCancelled(data: { bookingId: string }): Promise<void> {
...
async function notifyBookingAccepted(data: { bookingId: string }): Promise<void> {
  const admin = getSupabaseAdminClient();
  const { data: booking, error } = await admin
    .from("bookings")
    .select(
...
async function notifyDriverRideAccepted(data: { bookingId: string; seriesAcceptedCount?: number }): Promise<void> {
  const admin = getSupabaseAdminClient();
  const { data: booking, error } = await admin
    .from("bookings")
    .select(
...
async function notifyBookingUpdated(data: { bookingId: string }): Promise<void> {
  const admin = getSupabaseAdminClient();
  const { data: booking, error } = await admin
    .from("bookings")
...
async function notifyRideUnassigned(data: { bookingId: string; seriesAffectedCount?: number }): Promise<void> {
  const admin = getSupabaseAdminClient();
  const { data: booking, error } = await admin
    .from("bookings")
...
async function notifyDriverPatientCancelled(data: { bookingId: string }): Promise<void> {
  const admin = getSupabaseAdminClient();
  const { data: booking, error } = await admin
    .from("bookings")
...
async function notifyAdminNewDriverApplication(input: { driverDetailsId: string }): Promise<void> {
  const admin = getSupabaseAdminClient();
