-- =============================================================================
-- Mon Taxi Santé — Expose patient_phone / patient_birth_date for rebooking
--
-- When a patient (own live session, or recovered via reference + phone)
-- starts a brand new reservation, the app pre-fills the identity fields
-- from their most recent booking to avoid re-typing the same information.
-- get_my_bookings() and lookup_booking_by_reference() already return
-- patient_full_name for this purpose (see migration 009/010); this adds
-- patient_phone and patient_birth_date, the two remaining identity fields
-- needed by the booking form's Step 1, kept in sync between both functions
-- as established in migration 010.
-- =============================================================================
...
AS $$
  SELECT
    ...
    b.patient_phone, b.patient_birth_date,
