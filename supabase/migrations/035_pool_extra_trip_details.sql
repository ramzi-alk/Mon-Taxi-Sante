-- =============================================================================
-- Mon Taxi Santé — Détails de course supplémentaires dans le pool chauffeur
--
-- Avant d'accepter une course, un chauffeur doit pouvoir déplier la carte
-- pour voir des informations utiles à sa décision (PMT déclarée présente,
-- hospitalisation, séance d'une série), SANS jamais obtenir l'identité ou
-- le contact du patient avant acceptation (déjà masqués depuis la
-- migration 029 : patient_phone NULL, patient_full_name réduit au prénom).
--
-- pmt_declared / is_hospitalization / series_index / series_total ne sont
-- pas des données médicales ni des identifiants — contrairement à
-- medical_notes/mutual_name (toujours exclus de ces vues).
-- =============================================================================

-- ─── bookings_pool_for_drivers: + PMT / hospitalisation / série ───────────
...
COMMENT ON VIEW public.bookings_active_for_driver IS
  'Courses acceptées/en cours/terminées du chauffeur connecté (équivalent de fetchDriverRides), avec distance_to_driver_km, driver_rating_given (note que CE chauffeur a donnée au patient pour cette course, NULL = pas encore noté), patient_rating_received (note que le patient a donnée au chauffeur pour cette course, visible dès soumission), patient_rating_avg (note moyenne du patient toutes courses confondues) et pmt_declared/is_hospitalization/series_index/series_total (cohérence avec bookings_pool_for_drivers). L''adresse exacte de prise en charge reste masquée hors fenêtre de révélation (voir pickup_address_revealed, migration 031).';
